...
psims <- foreach(a = 1:nsim, .options.snow = opts) %dorng% {
  ...
  MSEmat.n5 = matrix(NA,nrow=5,ncol=length(tau_v))
  for(bb in 1:length(tau_v)){
    ...
    #### fill in MSE values
    MSEmat.n5[,bb]=c(tau,mean((Y.everyn-pred1)^2),mean((Y.everyn-pred2)^2),mean((Y.everyn-pred3)^2),mean((Y.everyn-pred4)^2))
  }
...
tau_v = outmat.n5[1,]
plot(tau_v, outmat.n5[2,], type="l", ylim=range(outmat.n5), xlab=expression(tau), ylab="MSPE", main="Nonlinear fixed effect, 5 Nodes", lwd = 2)
lines(tau_v, outmat.n5[3,], col="Blue", lty = 2, lwd = 2)
lines(tau_v, outmat.n5[4,], col="Red", lty = 3, lwd = 2)
lines(tau_v, outmat.n5[5,], col="green3", lty = 4, lwd = 2)