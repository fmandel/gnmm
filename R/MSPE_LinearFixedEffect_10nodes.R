...
psims <- foreach(a = 1:nsim, .options.snow = opts) %dorng% {
  ...
  MSEmat.l10 = matrix(NA,nrow=5,ncol=length(tau_v))
  for(bb in 1:length(tau_v)){
    ...
    #### fill in MSE values
    MSEmat.l10[,bb]=c(tau,mean((Y.everyn-pred1)^2),mean((Y.everyn-pred2)^2),mean((Y.everyn-pred3)^2),mean((Y.everyn-pred4)^2))
  }
  
  rbind(MSEmat.l10[1:5,])
...
tau_v = outmat.l10[1,]
plot(tau_v, outmat.l10[2,], type="l", ylim=range(outmat.l10), xlab=expression(tau), ylab="MSPE", main="Linear fixed effect, 10 Nodes", lwd = 2)
lines(tau_v, outmat.l10[3,], col="Blue", lty = 2, lwd = 2)
lines(tau_v, outmat.l10[4,], col="Red", lty = 3, lwd = 2)
lines(tau_v, outmat.l10[5,], col="green3", lty = 4, lwd = 2)
legend("topleft", c("GLMM","ANN","GNMM","MP"), col=c("Red","Blue","Black", "green3"), lty=c(3,2,1,4), lwd=2, cex=1, bty = 'n')