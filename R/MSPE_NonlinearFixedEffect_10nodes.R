## Simulation 2: MSPE, nonlinear fixed effect, 10 nodes ##
...
psims <- foreach(a = 1:nsim, .options.snow = opts) %dorng% {
  ...
  # Simulation 2: nonlinear, 10 nodes
  MSEmat.n10 = matrix(NA,nrow=5,ncol=length(tau_v))
  for(bb in 1:length(tau_v)){
    ...
    #### fill in MSE values
    MSEmat.n10[,bb]=c(tau,mean((Y.everyn-pred1)^2),mean((Y.everyn-pred2)^2),mean((Y.everyn-pred3)^2),mean((Y.everyn-pred4)^2))
  }
  
  rbind(MSEmat.n10[1:5,])
  
}

# calculate average MSPE
outmat.n10=Reduce("+", psims) / nsim

# MSPE plot (top right panel of Figure 2)
tau_v = outmat.n10[1,]
plot(tau_v, outmat.n10[2,], type="l", ylim=range(outmat.n10), xlab=expression(tau), ylab="MSPE", main="Nonlinear fixed effect, 10 Nodes", lwd = 2)
lines(tau_v, outmat.n10[3,], col="Blue", lty = 2, lwd = 2)
lines(tau_v, outmat.n10[4,], col="Red", lty = 3, lwd = 2)
lines(tau_v, outmat.n10[5,], col="green3", lty = 4, lwd = 2)