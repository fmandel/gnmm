...
psims <- foreach(a = 1:nsim, .options.snow = opts) %dorng% {
  ...
  MSEmat.n10 <- matrix(NA,nrow=6,ncol=length(tau_v))
  for(bb in 1:length(tau_v)){
    ...
    #### fill in MSE values
    MSEmat.n10[,bb] <- c(tau,mean((Y.everyn-pred1)^2),mean((Y.everyn-pred2)^2),mean((Y.everyn-pred3)^2),
                         mean((Y.everyn-pred4)^2),mean((Y.everyn-pred5)^2))
  }
  
  rbind(MSEmat.n10[1:6,])
...
tau_v <- outmat.n10[1,]
plot(tau_v, outmat.n10[2,], type='l', ylim=range(outmat.n10), xlab=expression(tau), ylab='MSPE', 
     main='Nonlinear fixed effect, 10 Nodes', lwd = 2)
lines(tau_v, outmat.n10[3,], col='Gray', lty = 2, lwd = 2)
lines(tau_v, outmat.n10[4,], col='Blue', lty = 3, lwd = 2)
lines(tau_v, outmat.n10[5,], col='Red', lty = 4, lwd = 2)
lines(tau_v, outmat.n10[6,], col='green3', lty = 5, lwd = 2)
