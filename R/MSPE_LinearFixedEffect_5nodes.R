...
psims <- foreach(a = 1:nsim, .options.snow = opts) %dorng% {
  ...
  MSEmat.l5 <- matrix(NA,nrow=6,ncol=length(tau_v))
  for(bb in 1:length(tau_v)){
    ...
    #### fill in MSE values
    MSEmat.l5[,bb] <- c(tau,mean((Y.everyn-pred1)^2),mean((Y.everyn-pred2)^2),mean((Y.everyn-pred3)^2),
...
tau_v <- outmat.l5[1,]
plot(tau_v, outmat.l5[2,], type='l', ylim=range(outmat.l5), xlab=expression(tau), ylab='MSPE', 
     main='Linear fixed effect, 5 Nodes', lwd = 2)
lines(tau_v, outmat.l5[3,], col='Gray', lty = 2, lwd = 2)
lines(tau_v, outmat.l5[4,], col='Blue', lty = 3, lwd = 2)
lines(tau_v, outmat.l5[5,], col='Red', lty = 4, lwd = 2)
lines(tau_v, outmat.l5[6,], col='green3', lty = 5, lwd = 2)
